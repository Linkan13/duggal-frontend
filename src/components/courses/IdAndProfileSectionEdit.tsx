...
export default function IdAndProfileSection({
  ...
  setStudentData: React.Dispatch<React.SetStateAction<TStudent | null>>;
}) {
  const [frontPreview, setFrontPreview] = useState<string | null>(
    studentData?.frontIdFile ? URL.createObjectURL(studentData.frontIdFile) : null
  );
  const [backPreview, setBackPreview] = useState<string | null>(
    studentData?.backIdFile ? URL.createObjectURL(studentData.backIdFile) : null
  );
  const [profilePreview, setProfilePreview] = useState<string | null>(
    studentData?.profilePicture ? URL.createObjectURL(studentData.profilePicture) : null
  );
  const [loading, setLoading] = useState(false);
  const [cameraTarget, setCameraTarget] = useState<CameraTarget>(null);

  // Helper to ensure state is not null
  const ensureStudentData = (): TStudent => {
    if (!studentData) {
      const newStudent: TStudent = {
        ...
        otherDocuments: [],
        courses: [],
        payment: { totalFees: 0, installments: [] },
        joinedDate: null,
      };
  ...
  const handleIdProofUpload = async (files: File[], side: "front" | "back") => {
    ...
    try {
      ...
      setStudentData((prev) => {
        const data = prev ?? ensureStudentData();
        if (side === "front") {
          const dobMatch = text.match(/\d{2}[/-]\d{2}[/-]\d{4}/);
          const dob = dobMatch ? dobMatch[0] : "";
          const aadhaarMatch = text.match(/\b\d{4}\s?\d{4}\s?\d{4}\b/);
          const idProofNumber = aadhaarMatch ? aadhaarMatch[0].replace(/\s/g, "") : "";
          return {
            ...data,
            frontIdFile: file,
  ...
  const handleProfileUpload = (file: File) => {
    setStudentData((prev) => {
      const data = prev ?? ensureStudentData();
      return { ...data, profilePicture: file };
    });
  ...
  return (
    <ComponentCard title="🆔 Aadhaar Card & Profile Picture">
      ...
      {/* Aadhaar Front/Back Section */}
      <div className="row g-4">
        {["front", "back"].map((side) => {
          const preview = side === "front" ? frontPreview : backPreview;
          const text =
            side === "front"
              ? studentData?.frontIdText
              : studentData?.backIdText;
      ...
      {cameraTarget && (
        <CameraCapture
          onCapture={(file) => {
            if (cameraTarget === "profile") handleProfileUpload(file);
            else handleIdProofUpload([file], cameraTarget);
          }}
